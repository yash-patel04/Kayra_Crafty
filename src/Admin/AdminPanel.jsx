...
const AdminPanel = ({ selectedArt }) => {
  ...
  useEffect(() => {
    if (selectedArt) {
      fetch(`${process.env.REACT_APP_API}/get/${selectedArt._id}`)
        .then((res) => {
          if (!res.ok) {
            throw new Error("Failed to fetch data");
          }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const url = `${process.env.REACT_APP_API}/update_details/${selectedArt.artId}/${selectedArt.productId}`;
    const method = "PATCH";
  ...
  const handleDelete = (productId) => {
    console.log("Delete clicked for product ID:", productId);
    fetch(`${process.env.REACT_APP_API}/delete/${selectedArt._id}/${productId}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (!res.ok) {
  ...
  return (
    <div className="admin-panel">
      ...
      <button onClick={handleAdd} className="add-product-button">
        +
      </button>
      ...
      {isModalOpen && (
        <div className="modal">
          <div className="modal-content">
            {/* <h2>Edit Product</h2> */}
            <form onSubmit={handleSubmit} className="product-form">
              {isEdit && <h2>Edit Product</h2>}
              {!isEdit && <h2>Add Product</h2>}
              <label htmlFor="productName">Product Name:</label>
              <input
                type="text"
                id="productName"
